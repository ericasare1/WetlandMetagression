...
ma_linear_scale <- stan("code/linearMA_bridgesampling.stan", 
				  pars = c("beta", "sigma", "gamma", "log_lik", "y_rep"), init = init,
				  data=data_stan_whole, iter=n_iter, chains=n_chains)#, seed = seed)
ma_linear_freshwl_scale <- stan("code/linearMA_bridgesampling.stan", 
						  pars = c("beta", "sigma", "gamma", "log_lik", "y_rep"),init = init,
						  data=data_stan_freshwl, iter=n_iter, chains=n_chains)#, seed = seed)
ma_linear_freshwl_can_scale <- stan("code/linearMA_bridgesampling.stan", 
							  pars = c("beta", "sigma", "gamma", "log_lik", "y_rep"),init = init,
							  data=data_stan_freshwl_can, iter=n_iter, chains=n_chains)#, seed = seed)
ma_linear_freshwl_us_scale <- stan("code/linearMA_bridgesampling.stan", 
							  pars = c("beta", "sigma", "gamma", "log_lik", "y_rep"),init = init,
...
fit_linear_fresh_us <- extract(ma_linear_freshwl_us)
fit_linear <- extract(ma_linear_)
...
y_prep_linfresh_us <- apply(fit_linear_fresh_us$y_rep, 2, mean) # extract the predicted y at their means : Linear freshwater model
y_prep_lin <- apply(fit_linear$y_rep, 2, mean) # extract the predicted y at their means : Linear freshwater model


linear_TE <- data.frame(cbind(y_prep_lin, df$lnwtp))
linear_TE <- linear_TE %>%
	mutate(wtp_y = exp(V2) - 1,
		   wtp_ypred = exp(y_prep_lin) - 1,
...
mean(linfreshwater_can_TE$TE)
min(linfreshwater_can_TE_noINF$TE)
max(linfreshwater_can_TE_noINF$TE)
sd(linfreshwater_can_TE_noINF$TE)

linfreshwater_us_TE <- data.frame(cbind(y_prep_linfresh_us, df_us_fresh$lnwtp))
