...
data_stan_whole <- list(N=nrow(df),
						...
						lwtp=df$lnwtp,
						#				  x=as.matrix(df[,6]),
						x=cbind(rep(1,nrow(df)),as.matrix(df[, 4:7]),
								as.matrix(df[, 10:14]),
								as.matrix(df[, 17:22])), 
						q0 = df$q0,
						q1 = df$q1)
...
data_stan_freshwl <- list(N=nrow(df_freshwl),
						  ...
						  #				  x=as.matrix(df[,6]),
						  x=cbind(rep(1,nrow(df_freshwl)),as.matrix(df_freshwl[, 5:6]),
						  		as.matrix(df_freshwl[, 10:14]),
						  		as.matrix(df_freshwl[, 17:22])), 
						  q0 = df_freshwl$q0,
						  q1 = df_freshwl$q1)
...
data_stan_freshwl_can <- list(N=nrow(df_canada_fresh),
							  ...
							  #				  x=as.matrix(df[,6]),
							  x=cbind(rep(1,nrow(df_canada_fresh)),as.matrix(df_canada_fresh[, 5:6]),
							  		as.matrix(df_canada_fresh[, 10:14]),
							  		as.matrix(df_canada_fresh[, 17:21])), 
